...
export default function AddTestModal({
  ...
  const [zoneCount,setZoneCount] =
    useState<ZoneType>("5")

  const [inputMode,setInputMode] =
    useState<"time"|"pace">("time")
...
const [css200Sec,setCss200Sec] = useState("")
  ...
  /* 🔥 METHODS לפי SPORT */
  const methodOptions =
    Object.keys(ZONES_CONFIG[sport] || {}) as MethodType[]

  /* 🔥 TESTS לפי SPORT+METHOD */
  const testOptions =
    Object.keys(
      ZONES_CONFIG[sport]?.[method]?.tests || {}
    )
  ...
  const calculate = ()=>{
    ...
    if(sport!=="bike"){
      ...
      pace = total / dist
        ...
        pace =
          Number(paceMin)*60 + Number(paceSec)
  ...
  if(showPower){
    result.power = customZones.power.map((f:number)=> Number(power) * (f/100))
  }
  ...
  const t400 =
    Number(css400Min)*60 + Number(css400Sec)

  const t200 =
    Number(css200Min)*60 + Number(css200Sec)
  ...
  const submit = ()=>{
    ...
    value = Math.round(zones.pace[3] || zones.pace[zones.pace.length-1] || 0)
  }else{
      value =
        Number(minutes)*60 + Number(seconds)
    }
  ...
  const showPace = sport !== "bike" && method !== "max" && !(sport==="swim" && testType==="css")
...
placeholder={
  ...
  <select
    value={zoneCount}
    onChange={e=>
      setZoneCount(e.target.value as ZoneType)
    }
  >
