...
export default function ProgressChart({ tests, sport }: Props) {
  ...
  const pb = isLowerBetter ? Math.min(...values) : Math.max(...values);
  ...
  const formatValue = (v: number) => {
    return sport === "bike" ? `${Math.round(v)}w` : secondsToTime(v);
  };
    ...
    <div className="chart-container">
      <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
        ...
        <polyline
          ...
          stroke="#2563eb"
          strokeWidth="2"
          points={sorted.map((t, i) => `${getX(i)},${getY(t.value)}`).join(" ")}
        />
      ...
      {hovered !== null && (
        <div className="chart-tooltip">
          <div>{formatValue(sorted[hovered].value)}</div>

          <div>{new Date(sorted[hovered].date).toLocaleDateString()}</div>
        </div>
      )}
